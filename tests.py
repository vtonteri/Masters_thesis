...
def visualize_embeddings(encoder, dataloader, n_samples, device, algo):
    n = 0
    codes, labels = [], []
    with torch.no_grad():
    ...
    if codes.shape[1] > 2:
        ...
        if algo =="spectral":
            print("Use Spectral Embedding")
            codes = SpectralEmbedding().fit_transform(codes)
    labels = np.hstack(labels)

    colors = [
        'yellow', 'pink', 'grey', 'lightblue', 'black', 'red', 'gold', 'palegreen', 'blue',
        'lightcoral', 'orange', 'mediumturquoise', 'violet'
    ]
